...
const Courses: React.FC = () => {
  ...
  return (
    <section id="courses" className="py-12 bg-gray-100 mt-10">
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap justify-center gap-2 md:gap-4 mb-8">
          {Object.keys(courseCategories).map((category) => (
            <button
              key={category}
              className={`px-4 py-2 rounded transition-colors duration-300 font-medium shadow-md hover:bg-teal-700 hover:text-white ${
                activeCategory === category
                  ? "bg-teal-800 text-white"
                  : "bg-white text-teal-500"
              }`}
              onClick={() =>
                setActiveCategory(category as CourseCategory)
              }
            >
              {category}
            </button>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
          <input
            ...
            className="px-4 py-2 border rounded-lg shadow-md w-full md:w-1/2 focus:ring-2 focus:ring-teal-500"
          />

          <select
            value={selectedDifficulty}
            onChange={(e) => setSelectedDifficulty(e.target.value)}
            className="w-full md:w-auto px-4 py-2 border rounded-lg shadow-md focus:ring-2 focus:ring-teal-500"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((course) => (
            ...
            >
              ...
              <a
                href={course.link}
                className="inline-block mt-4 px-4 py-2 bg-teal-600 text-white font-medium rounded-lg shadow-md hover:bg-teal-700 transition duration-300"
              >
                View Course
              </a>
            </div>
